...
def prepare_dataset(voxel_size: float, first: np.ndarray, second: np.ndarray):
    """ Prepare a dataset for point cloud registration

    :param voxel_size: The size of the voxel
    :param first: Dataset 1
    :param second: Dataset 2
    :return: ...
    """
...
def main(source_array: np.ndarray, target_array: np.ndarray, voxel_size: float = 0.005) -> np.ndarray:
    source, target, source_down, target_down, source_fpfh, target_fpfh = prepare_dataset(voxel_size, source_array, target_array)

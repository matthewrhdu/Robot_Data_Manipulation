...
from ros_packages.in_hand_control.object_detection.Algorithms_Builtins import *


def segment_point_cloud(data_source: Union[str, np.ndarray], use_ransac: bool = True, to_save: bool = False,
    ...
    filtered_clusters, clusters = run_kmeans(data_points, 2, threshold=500)
    ...
    for n, points in enumerate(filtered_clusters):
        try:  # Catching RuntimeError that can be raised by bounding box Algorithm
            ...
            # Saving points
            if to_save:
                np.save(f"object_{n}.npy", points)
                np.save(f"object_{n}_x.npy", points[:, 0])
                np.save(f"object_{n}_y.npy", points[:, 1])
                np.save(f"object_{n}_z.npy", points[:, 2])
    ...
    transformation = [(center[0], center[1], center[2]) for center in centers]

...
def main(filename):
    ...
    transformation_matrix = np.load("Data/bounding_box_transform_matrix.npy")

    for points in filtered_clusters:
        bounding_box = draw_bounding_box(points, OrientedBoundingBox)
        box_points = np.array([np.matmul(transformation_matrix, point) for point in np.array(bounding_box.get_box_points())])
        box_center = np.matmul(transformation_matrix, np.array(bounding_box.get_center()))
